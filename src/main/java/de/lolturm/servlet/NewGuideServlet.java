	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (speichern.equals("Speichern")) {
			try {
				...
				guideBean.setChampion(LTChampions.get(Integer.parseInt(champion)));
				guideBean.setPosition(PositionEnum.getByPosition(position));
				LTItemSetBean LTItemSetBean = new LTItemSetBean();
				String itemset = LTItemSetBean.getItemSetString(slot1, slot2, slot3, slot4, slot5, slot6, slot7);
				guideBean.setItemSet(LTItemSetBean.setItemSetArray(itemset));
				...
				if (useridBean.getId() == null) {
					response.sendRedirect("/LoginServlet?comeFrom=NewGuideServlet");
				} else {
					...
					if (!Guides.checkGuide()) {
						...
						guideBean.setId(id);
						response.sendRedirect("/jsp/NewGuide.jsp");
					} else {
						Guides.updateGuide();
						response.sendRedirect("/jsp/NewGuide.jsp");
					}
